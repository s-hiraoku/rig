...
def create_mcp_server() -> FastMCP:
    ...
    def rig_delegate(
        ...
        """Delegate a task to a configured coding agent.
    ...
    def rig_patch_create(
        ...
        """Delegate a task in an isolated worktree and capture a patch."""
        return run_tool(
            task=task,
            agent=agent,
            task_file=task_file,
    ...
    @server.tool()
    def rig_history(cwd: str | None = None) -> dict[str, Any]:
    ...
    def rig_list_agents(cwd: str | None = None) -> dict[str, Any]:
        """List configured agents available for rig_delegate.

        Returns { ok, default_agent, agents } where each agent includes name,
        command, args, and whether it is the default.
        """
        return list_agents_tool(cwd=cwd)

    @server.tool()
    def rig_history_show(
    ...
    def rig_patch_show(
        ...
        """Read diff.patch for a patch run.

        Use this after rig_patch_create to inspect changes before applying
        them. Returns { ok, run_id, diff_path, diff }.
        """
...
def get_run_tool(*, run_id: str = "latest", cwd: str | None = None) -> dict[str, Any]:
    ...
    return {"ok": True, "run": run, "result": store.read_result(run)}
    ...
    *, run_id: str = "latest", cwd: str | None = None
    ...
    if os.environ.get("RIG_MCP_ALLOW_APPLY") != "1":
        return error_response(
            "rig_patch_apply is disabled. Set RIG_MCP_ALLOW_APPLY=1 to enable it."
        )
    try:
        store = store_for(cwd)
        run = store.resolve_run(run_id)
